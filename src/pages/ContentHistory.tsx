...
import { getHistory, getN8nExecutionUrl } from "@/lib/n8nApi";
...
const mockContents: ContentItem[] = [
  {
    ...
    metaDescription:
      "Aprenda as técnicas essenciais de SEO para posicionar seu site no topo do Google.",
    content: "",
    source: "youtube",
    keyword: "SEO 2024",
    status: "published",
...
export function ContentHistory() {
  ...
  const contents: ContentItem[] =
    apiContents.length > 0 ? apiContents : mockContents;
  ...
  const filteredContents = contents.filter((content) => {
    ...
    const matchesFilter =
      activeFilter === "all" || content.status === activeFilter;
  ...
  const statusCounts = {
    ...
    reviewed: contents.filter((c) => c.status === "reviewed").length,
          ...
          {filteredContents.length > 0 ? (
            filteredContents.map((content) => (
              <ContentCard
                ...
                executionUrl={
                  content.executionId
                    ? getN8nExecutionUrl(content.executionId)
                    : null
                }
              />
            ))
